...
- File > Startup Impact lists every startup entry: the Run registry keys (user,
  machine and 32-bit machine), both Startup folders, services set to start
  automatically, and scheduled tasks triggered at logon or boot. Each shows
  whether it is enabled, how many of its processes are running, and the CPU time
  and private memory they have used, heaviest first. Go to Process opens the
  running process; Open file location reveals the executable. Processes that
  cannot be opened are still counted, by name, which can credit a different
  program of the same name; services are matched by the pid the service manager
  reports.
